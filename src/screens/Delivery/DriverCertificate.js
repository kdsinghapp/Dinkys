...
useEffect(()=>{
    ...
    if(user){
        ...
        setInsuranceImage({uri:user?.driver_details_data?.insurance_image})
        setLicenseImage({uri:user?.driver_details_data?.driving_license_image})
        setPeopleTransportImage({uri:user?.driver_details_data?.transport_license_image})
...
console.log('user?.driver_details_data?.insurance_image',user?.driver_details_data?.insurance_image);
...
fetch(`${DOMAIN}save-driver-details`, requestOptions)
  ...
  return (
    ...
    */}
      ...
      <ProfileHeader name="Certificate" Dwidth="36%" />
